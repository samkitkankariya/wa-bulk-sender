...
async function typeAndSend(message, attachment) {
  ...
  if (attachment) {
    ...
    try {
      ...
      console.log('[WA Sender] Paste event dispatched, waiting for preview modal...');
      await sleep(2500); // Wait for modal to render

      // We are ignoring the caption box entirely as requested. Send the image upfront.
      ...
      // Random delay between sending image and sending text (2 to 4.5 seconds)
      if (message) {
        const randomDelay = 2000 + Math.random() * 2500;
        console.log(`[WA Sender] Image sent. Waiting ${Math.round(randomDelay)}ms before typing message...`);
        await sleep(randomDelay);

        // Refresh mainBox reference since DOM might have shifted
        mainBox = document.querySelector('[data-testid="conversation-compose-box-input"]') || mainBox;
      }
    } catch (e) {
...
function getJob() {
  return new Promise(resolve => chrome.storage.session.get('job', resolve));
}

function saveJob(job) {
  return new Promise(resolve => chrome.storage.session.set({ job }, resolve));
}
